...
2. **Workflow rút gọn cho reviewer (dùng `cli/`):**
   ```bash
   python cli/prepare_semantic_dataset.py   # gộp Step 2 + 4
   python cli/run_full_eval.py              # Step 5
   python cli/run_train_test_eval.py        # Train/Test split 80/20
   ```

Sau khi chạy Step 4 (hoặc `cli/prepare_semantic_dataset.py`), các file `processed_data.csv`, `ground_truth_labels.csv`, `semantic_dataset.csv` sẽ có trong `data/processed/`.
...
| Section III-B – Ground-truth synthesis (NDI/PDI tags) | `cli/prepare_semantic_dataset.py`, `src/ground_truth.py` | Pipeline raw → processed → semantic dataset, sử dụng đúng rule list trong paper. |
...
| Step 5 – Evaluate toàn bộ dataset | `cli/run_full_eval.py` | Tính Accuracy, F1, Precision, Recall, confusion matrix, classification report. |
| Step 6 – Train/Test split 80/20 | `cli/run_train_test_eval.py` | Stratified split, export kết quả JSON + hình + report cho train/test. |
...
```bash
python cli/run_train_test_eval.py
```
Artefact sinh ra:
...
## 7. Tùy chỉnh
- Tắt thresholds: `FSE_ENABLE_THRESHOLDS=0 python cli/run_full_eval.py`
- Tắt fallback: `FSE_ENABLE_FALLBACK=0 python cli/run_full_eval.py`
